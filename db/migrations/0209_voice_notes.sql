-- Phase 3: explicit inbound voice-note transcription with metadata-only audit.
...
CREATE OR REPLACE FUNCTION reject_voice_note_stt_event_mutation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_voice_note_stt_events_immutable
    ...
    FOR EACH ROW
    EXECUTE FUNCTION reject_voice_note_stt_event_mutation();
