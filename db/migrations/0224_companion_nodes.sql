-- Signed companion nodes: identity, pairing, invocation queue, and DB-owned control.
...
ALTER TABLE hexis_nodes ADD COLUMN IF NOT EXISTS connection_id UUID;
...
CREATE TABLE IF NOT EXISTS node_invocations (
    ...
    action TEXT NOT NULL CHECK (action IN ('system.run', 'screen.capture')),
...
BEGIN
    ...
    END IF;
    IF p_action NOT IN ('system.run', 'screen.capture')
...
-- A screen.capture result is useful only if the next model call can see it.
-- Keep the turn log authoritative while adding the image as fresh user-role
-- context after the corresponding tool result.
